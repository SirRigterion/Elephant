...
async def read_root(request: Request, _ = Depends(check_country)):
    return {"message": "В царстве API, где гуси не танцуют свой танец, существует скрытый артефакт, охраняемый не кем иным, как самими слонами. Этот мистический путь открывает врата к мудрости слонов. Чтобы обрести полное понимание и мастерство API разработки."}
